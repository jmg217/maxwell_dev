...
//Provides interface with three dimensional matrices stored as one dimensional arrrays
double* three_dim_index(double* matrix, int i, int j, int k, double m, int b, int num_assets);

//Provides interface with two dimensional matrices stored as one dimensional arrrays stored on the GPU
...
//Mesh Generation Kernel. This kernel uses the independent path construction.
