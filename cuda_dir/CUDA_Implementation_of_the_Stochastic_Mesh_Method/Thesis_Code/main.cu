...
//This function generates the mesh weights
...
//This function converts a one dim vector to a one dim array
void one_dim_array(std::vector< double > &vals, double array[], int N);

//this function provides us with an indexing interface for three dim arrays
__host__ __device__ double* three_dim_index(double* matrix, int i, int j, int k, double m, int b, int num_assets);

//this function provides us with an indexing interface for two dim arrays
__host__ __device__ double* two_dim_index(double* vector, int i, int j, double m, int b);

//this function calculates the high bias option price.
...
//this function calculates the low bias option price
...
//this function outputs the payoff values of a one dimensional option a txt file. 
void print_high_payoff(int b, double m, double* X, double* V, double asset_amount[], double* W );

//this function outputs the simulated stock price paths to a txt file.
void SimulationPaths(int b, double m, double* X, double* V, double asset_amount[], double* W, double x );

//This function initialises the seeds on each thread for the random number generator
...
//Read in all input parameters from settings.txt
...
// initialise arrays for the mesh, weights and values
int m_int= (int)m;
...
for(int iterator=0; iterator<N; iterator++){
	...
	//This is the mesh estimator function. It will return a high bias price
	...
	//This is the path estimator function. It will return a low bias price.
...
//If the dimension of the option=1 we call the following functions. These two function produce the txt files 'OneDimHighPayoff.txt' and 'stocksimulation.txt' which are used for plots of the one dimensional payoff and the simulated stock price paths. 
if(num_assets==1){
...
//output latex format results to latexresults.txt
